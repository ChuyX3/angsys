...
async_msg_task::async_msg_task(activity* parent, core::async::cond_t cond, core::async::mutex_t mut, events::message_t msg)
...
ANG_IMPLEMENT_CLASSNAME(ang::platform::android::async_msg_task);
ANG_IMPLEMENT_OBJECTNAME(ang::platform::android::async_msg_task);
...
bool async_msg_task::wait(core::async::async_action_status_t st, dword ms)const
{
	...
	activity_t activity = _parent.lock();
	if (activity.is_empty())
		return false;

	if (activity->is_worker_thread())
