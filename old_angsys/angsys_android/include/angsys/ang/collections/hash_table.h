...
namespace ang
{
	...
	{
		template<typename K, typename T> class hash_table_data;
		template<typename K, typename T> using hash_table = object_wrapper<hash_table_data<K, T>>;
	}
	...
	namespace collections
	{
		...
		struct hash_table_node
		{
			...
			hash_table_node(K key) : _pair(key){}
			hash_table_node(K key, value_t val) : _pair(ang::move(key), ang::move(val)) {}
			hash_table_node(pair_t p) : _pair(ang::move(p)) {}
		...
		public:
			...
			typedef hash_table_data<K, T>				self_t;
		...
		public:
			...
			inline hash_table_data(std::initializer_list<pair<K, T>> list);
		...
		private:
			inline iterator<pair<K, T>> find_index(index idx)const override { return iterator_t(const_cast<hash_table_data<K, T>*>(this), null); }
			inline index index_of(iterator<pair<K, T>> it)const override { return -1; }
			inline iterator<pair<K, T>> find(const pair<K, T>&, bool)const override { return iterator_t(const_cast<hash_table_data<K, T>*>(this), null); }
			inline iterator<pair<K, T>> find(const pair<K, T>&, iterator<pair<K, T>>, bool)const override { return iterator_t(const_cast<hash_table_data<K,T>*>(this), null); }
		...
		uint LINK hash_table_get_next_size(uint);
	...
	template<typename K, typename T>
	class object_wrapper<collections::hash_table_data<K,T>>
	{
	public:
		typedef collections::hash_table_data<K,T> type;

	private:
		collections::hash_table_data<K,T>* _ptr;

	public:
		object_wrapper();
		object_wrapper(std::nullptr_t const&);
		object_wrapper(collections::hash_table_data<K,T>*);
		...
		object_wrapper(object_wrapper<collections::hash_table_data<K,T>> &&);
		object_wrapper(object_wrapper<collections::hash_table_data<K,T>> const&);
		~object_wrapper();

	public:
		void clean();
		bool is_empty()const;
		collections::hash_table_data<K,T>* get(void)const;
		void set(collections::hash_table_data<K,T>*);
		collections::hash_table_data<K,T> ** addres_of(void);

	public:
		object_wrapper& operator = (object_wrapper<collections::hash_table_data<K,T>> &&);
		object_wrapper& operator = (object_wrapper<collections::hash_table_data<K,T>> const&);
		object_wrapper& operator = (std::nullptr_t const&);
		object_wrapper& operator = (collections::hash_table_data<K,T>*);
		...
		object_wrapper_ptr<collections::hash_table_data<K,T>> operator & (void);
		collections::hash_table_data<K,T> * operator -> (void);
		collections::hash_table_data<K,T> const* operator -> (void)const;
		explicit operator collections::hash_table_data<K,T> * (void);
		explicit operator collections::hash_table_data<K,T> const* (void)const;
	...
	static void foreach(collections::hash_table<K,T> const& store, F func) {
