...
pointer buffer::operator new(wsize)throw()
{
	return null;
}
...
pointer aligned_buffer::operator new(wsize)throw()
{
	return null;
}
...
{
	return object::auto_release(16u);//to call aligned deallocation
}
...
wsize aligned_buffer::mem_copy(wsize size, pointer ptr)
{
	wsize sz = min(size, buffer_size());
...
pointer aligned_buffer::map_buffer(windex start, wsize size)
{
	if ((start + size) > buffer_size())
		return null;
	return (byte*)buffer_ptr() + start;
...
void aligned_buffer::unmap_buffer(pointer ptr, wsize used)
{
}
...
pointer static_buffer::operator new(wsize)throw()
{
	return null;
}

void static_buffer::operator delete(pointer)throw()
{
