...
template<class T>
ang::collections::base_iterator<T>::base_iterator(ang::collections::ienum<T>* par, ang::collections::position_t cur, uint off)
	: _parent(par)
	, _current(cur)
	, _offset(off)
{
...
template<class T>
inline uint ang::collections::base_iterator<T>::offset()const
{
	return _offset;
}

template<class T>
inline void ang::collections::base_iterator<T>::offset(uint c)
{
	_offset = c;
}
