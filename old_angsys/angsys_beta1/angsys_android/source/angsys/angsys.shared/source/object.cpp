...
#define ANG_ALLOCATOR_ALLOC(_SIZE) ang::memory::allocator_manager::get_allocator(ang::memory::allocator_manager::object_allocator)->malloc(_SIZE)
#define ANG_ALLOCATOR_ALLOC_DEBUG(_SIZE, _FILE, _LINE) ang::memory::allocator_manager::get_allocator(ang::memory::allocator_manager::object_allocator)-> malloc(_SIZE, _FILE, _LINE)
#define ANG_ALLOCATOR_RELEASE(_PTR) ang::memory::allocator_manager::get_allocator(ang::memory::allocator_manager::object_allocator)->free(_PTR)
#define ANG_ALIGNED_ALLOCATOR_ALLOC(_SIZE, _ALIGMENT) ang::memory::allocator_manager::get_allocator(ang::memory::allocator_manager::aligned_allocator)->malloc(_SIZE)
#define ANG_ALIGNED_ALLOCATOR_RELEASE(_PTR) ang::memory::allocator_manager::get_allocator(ang::memory::allocator_manager::aligned_allocator)->free(_PTR)
#else
...
{
	smart_ptr_info_ptr_t ptr = (smart_ptr_info_ptr_t)ANG_ALIGNED_ALLOCATOR_ALLOC(size + sizeof(smart_ptr_info_t), ALIGMENT);
	ptr->_shared_counter = 0;
	ptr->_ref_counter = 0;
	ptr->_object = (object*)ang_void_ptr_t(wsize(ptr) + sizeof(smart_ptr_info_t));
	return ptr->_object;
...
{
	throw exception(except_code::invalid_access);
	return null;
}
...
void object::operator delete(ang_void_ptr_t ptr, ushort)
{
	...
	info->_object = null;
	if (info->_shared_counter == 0)
		ANG_ALIGNED_ALLOCATOR_RELEASE(info);
}
...
bool object::auto_release()
{
	...
	if (r <= 0U)
	{
		ANG_ALLOCATOR_RELEASE(GET_SMART_PTR_INFO(this));
	}
	return true;
}
...
bool object::auto_release(ushort)
{
	...
	if (r <= 0U)
	{
		ANG_ALIGNED_ALLOCATOR_RELEASE(GET_SMART_PTR_INFO(this));
	}
	return true;
}
