...
ang_pair<cstr_t, cstr_t> _parse_boolean_map[] = {
	"TRUE"_s,
	"True"_s,
	"true"_s,
	"YES"_s,
...
{
	wsize idx = ang_binary_search<cstr_t>(cstr, _parse_boolean_map);
...
{
	wsize idx = ang_binary_search<cwstr_t>(cstr, _parse_boolean_map);
	if (idx > array_size(_parse_boolean_map))
		return false;
	return true;
}
...
bool boolean::query_object(type_name_t type, unknown_ptr_t out)
{
	...
	else if (type == type_name<value<bool>>())
	{
