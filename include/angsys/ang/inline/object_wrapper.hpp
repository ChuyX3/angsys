...
template<typename T>
inline ang::object_wrapper<ang::value_wrapper<T>>& ang::object_wrapper<ang::value_wrapper<T>>::operator = (ang::value<T> const& val)
{
	if (_ptr)
		_ptr->set(val);
	else 
...
template<typename T>
inline ang::object_wrapper_ptr<T> ang::object_wrapper<ang::value_wrapper<T>>::operator& (void)
{
	return this;
}
...
ang::type_name_t ang::value_wrapper<T>::class_name() {
	static string _type_name = ang::move("ang::value_wrapper<"_o + type_name<T>() + ">"_s);
