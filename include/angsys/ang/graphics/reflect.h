...
#elif !defined __ANGRAPH_REFLECT_H__
#define __ANGRAPH_REFLECT_H__
	...
	namespace graphics
	{
		namespace reflect
		{
			...
			public:
				...
				bool load(xml::xml_node_t, uint aligment = -1);
			...
			public:
				...
				variable(static_array<byte> bytes, variable_desc desc, uint aligment = invalid_index);

				uint aligment()const;
				static_array<byte> raw_data()const;
				...
				template<typename T> inline auto array_cast() -> decltype(variable_cast<ang::static_array<T>>::cast(this)) {
					return variable_cast<ang::static_array<T>>::force_cast(this);
				}
...
template<typename T> struct ang::graphics::reflect::variable::variable_cast<ang::static_array<T>>
{
	static ang::static_array<T> force_cast(ang::graphics::reflect::variable* var) {
		wsize size = var->_raw_data.size() / get_memory_size_aligned_less(sizeof(T), var->descriptor().aligment());
		return ang::static_array<T>((T*)var->_raw_data.get(),  min(size, var->descriptor().array_count()));
	}
	static ang::static_array<T> cast(ang::graphics::reflect::variable* var) {
		return ang::static_array<T>((T*)var->_raw_data.get(), var->descriptor()->array_count);
	}
	static bool is_type_of(variable* var) { return variable_cast<T>::is_type_of(var); }
};


template<> struct LINK ang::graphics::reflect::variable::variable_cast<ang::static_array<ang::graphics::reflect::variable>>
{
...
template<> inline ang::graphics::reflect::var_type_t ang::xml::xml_value::as<ang::graphics::reflect::var_type_t>()const { return ang::graphics::reflect::var_type_t::parse(as<cwstr_t>()); }
template<> inline ang::graphics::reflect::var_class_t ang::xml::xml_value::as<ang::graphics::reflect::var_class_t>()const { return ang::graphics::reflect::var_class_t::parse(as<cwstr_t>()); }
template<> inline ang::graphics::reflect::var_semantic_t ang::xml::xml_value::as<ang::graphics::reflect::var_semantic_t>()const { return ang::graphics::reflect::var_semantic_t::parse(as<cwstr_t>()); }

#endif//__ANGRAPH_REFLECT_H__