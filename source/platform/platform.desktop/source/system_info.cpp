...
foundation::size<float> system_info::main_screen_bound()
{
	foundation::size<float> size((float)GetSystemMetrics(SM_CXSCREEN), (float)GetSystemMetrics(SM_CYSCREEN));
	return size;
}

foundation::size<float> system_info::current_screen_bound()
{
	auto app = app::current_app();
	if (app != null && app->main_wnd() != null)
	{
		auto monitor = MonitorFromWindow((HWND)app->main_wnd()->get_core_view_handle(), MONITOR_DEFAULTTONEAREST);
		if (monitor != null)
		{
			MONITORINFO info;
			if (GetMonitorInfo(monitor, &info) != FALSE)
			{
				return foundation::size<float>(float(info.rcMonitor.right - info.rcMonitor.left), float(info.rcMonitor.bottom - info.rcMonitor.top));
			}
		}
	}
