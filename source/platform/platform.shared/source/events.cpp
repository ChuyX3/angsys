...
void* ievent_function::operator new(wsize_t size)
{
	return memory::allocator_manager::get_allocator(0)->memory_alloc(size);
}

void ievent_function::operator delete(void* ptr)
{
	memory::allocator_manager::get_allocator(0)->memory_release(ptr);
}
...
bool event::is_child_of(type_name_t name)
{
	return (name == ang::type_name<event>()
		|| object::is_child_of(name)
		|| ievent::is_child_of(name));
}
...
bool event::query_object(ang::type_name_t name, ang::unknown_ptr_t out)
{
	...
	else if (name == ang::type_name<ievent>())
	{
		*out = static_cast<ievent*>(this);
		return true;
	}
	return false;
}
...
int event_listener::invoke(platform::events::imsg_event_args_t args)const
{
	if (!is_empty())
	{
		...
		foreach(_functions.get(), [&](event_t const & e) {
