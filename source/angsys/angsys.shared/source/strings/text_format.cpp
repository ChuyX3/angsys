...
bool format_parser::format(qword flags, string& out)
{
	...
	case text_format::text:
		...
		out += "}"_s;
		break;
	case text_format::signed_interger:
		out += "{signed:"_s;
		...
		break;
	case text_format::usigned_interger:
		out += "{unsigned:"_s;
...
{
	flag = (qword)text_format::target::signed_interger;
...
{
	flag = (qword)text_format::target::usigned_interger;
