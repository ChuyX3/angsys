...
namespace ang
{
	...
	template<class T1, class T2>
	inline index string_find_any_of(T1 const* cstr, uint lenght, array<T2> const& arr, index start) {

		if (cstr == null || arr.counter() == 0)
			return invalid_index;
		...
		T2 const* chars = arr.Data();
		index i = start, j = 0;
		do {
			j = 0;
			while (j < arr.counter() && chars[j] != *beg) ++j;
			if (j != arr.counter()) return i;
			i++;
		} while (++beg < end);
		return invalid_index;
	}
