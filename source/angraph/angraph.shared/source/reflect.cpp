...
bool variable_desc::load(xml::xml_node_t input, uint aligment)
{
	if (input.is_empty())
		return false;
	...
	{
		foreach(atts, [this](xml::xml_attribute_t att)
		{
			auto name = att->xml_name().as<cwstr_t>();
			if (name == "name"_s)
				_var_name = att->xml_value();
			else if (name == "array"_s)
				_array_count = att->xml_value().as<uint>();
			else if (name == "aligment"_s)
				_aligment = att->xml_value().as<uint>();
		});
	...
	else if (!atts.is_empty())
	{
		foreach(atts, [this](xml::xml_attribute_t att)
		{
			...
			if (name == "name"_s)
				...
				_var_type = att->xml_value().as<var_type_t>();
			else if (name == "class"_s)
				_var_class = att->xml_value().as<var_class_t>();
			else if (name == "array"_s)
				_array_count = att->xml_value().as<uint>();
			else if (name == "aligment"_s)
				_aligment = att->xml_value().as<uint>();
...
bool variable_desc::save(xml::xml_builder_t)const
{
	return false;
}
...
wsize variable_desc::calculate_positions(bool recursive)
{
	...
	if (recursive)
	{
		...
		wsize temp = 0;
		wsize res = 0;
		foreach(_fields, [&](variable_desc& desc) {
...
wsize variable_desc::get_size_in_bytes()const
{
	...
	case var_type::block: {
		...
		if (_fields.is_empty())
			return 0;
		variable_desc& desc = *_fields->end();
...
wsize variable_desc::get_size_in_bytes(uint aligment)const
{
	...
	case var_type::block: {
		...
		wsize temp = 0;
		wsize res = 0;
		foreach(_fields, [&](variable_desc const& desc) {
...
bool attribute_desc::load(xml::xml_node_t inputs, collections::vector<attribute_desc>& out)
{
	...
	foreach(inputs->xml_children(), [&](xml::xml_node_t input)
	{
		if(desc.load(input))
			out += desc;
	});
...
bool attribute_desc::load(xml::xml_node_t input)
{
	if (input.is_empty() || !input->xml_has_attributes())
		return false;
	auto att = input->xml_attributes();
	_var_name = att["name"];
	_var_type = att["type"].as<var_type_t>();
	_var_class = att["class"].as<var_class_t>();
	_semantic = att["semantic"].as<var_semantic_t>();
	_semantic_index = att["semantic_idx"].as<uint>();
	return true;
}

bool attribute_desc::save(xml::xml_builder_t)const
{
	return false;
}
...
wsize_t attribute_desc::get_size_in_bytes()const
{
	switch (_var_type.get())
	{
	case var_type::s16:
