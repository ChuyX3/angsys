...
namespace ang
{
	...
	{
		template<typename T, template<typename> class allocator = memory::default_allocator> class queue_object;
		template<typename T, template<typename> class allocator = memory::default_allocator> using queue = object_wrapper<queue_object<T, allocator>>;
	}

	namespace collections
	{
		/******************************************************************/
		/* template class queue_object :                                 */
		/*  -> implements the buffer memory of queue object              */
		/******************************************************************/
		template<typename T, template<typename> class allocator>
		class queue_object final
			: public object
			, public isequence<T>
		{
		public:
			typedef T							type;
			typedef queue_object<T, allocator>	self_t;
		...
		public:
			inline queue_object();
			template<typename U>inline queue_object(ang::initializer_list_t<U> list);
			inline queue_object(const ang::nullptr_t&);
			inline queue_object(queue_object&& ar);
			inline queue_object(const queue_object& ar);
			inline queue_object(const queue_object* ar);
			inline queue_object(const ienum_t* store);
		...
		public: //methods
			...
			inline void clean();
			inline bool move(queue_object<T, allocator>&);
		...
		template<>
		class queue_object<void> final {};
	}

	/******************************************************************/
	/* template class ang::object_wrapper<queue_object> :            */
	/*  -> specialization of object_wrapper<queue_object> -> queue  */
	/******************************************************************/
	template<typename T, template<typename> class allocator>
	class object_wrapper<collections::queue_object<T, allocator>>
	{
	public:
		typedef collections::queue_object<T, allocator> type;
		typedef typename collections::queue_object<T, allocator>::type data_type;

	private:
		collections::queue_object<T, allocator>* _ptr;
	...
	public:
		...
		void clean_unsafe();
		bool is_empty()const;
		collections::queue_object<T, allocator>* get(void)const;
		void set(collections::queue_object<T, allocator>*);
		collections::queue_object<T, allocator>** addres_of(void);
	...
	public:
		object_wrapper& operator = (collections::queue_object<T, allocator>*);
		...
		object_wrapper& operator += (T const& value) {
			if (is_empty()) set(new collections::queue_object<T, allocator>());
			_ptr->push(value);
			return *this;
		}

		object_wrapper_ptr<collections::queue_object<T, allocator>> operator & (void);
		collections::queue_object<T, allocator> * operator -> (void);
		collections::queue_object<T, allocator> const* operator -> (void)const;
		explicit operator collections::queue_object<T, allocator> * (void);
		explicit operator collections::queue_object<T, allocator> const* (void)const;
	};
