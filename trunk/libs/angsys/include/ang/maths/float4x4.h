/*********************************************************************************************************************/
/*   File Name: ang/maths/float4x4.h                                                                                 */
...
namespace ang
{
	namespace maths
	{
		...
		struct float4x4
		{
			...
			template<uint> float4& get() { throw_exception(except_code::array_overflow); }
			template<uint> float4 const& get()const { throw_exception(except_code::array_overflow); }
			template<uint> void set(float4 const& value) { throw_exception(except_code::array_overflow); }

			template<uint X, uint Y> float get()const { return get<X>().template get<Y>(); }
			template<uint X, uint Y> void set(float value) { get<X>().template set<Y>(value); }
		...
		struct double4x4
		{
			...
			template<uint> double4& get() { throw_exception(except_code::array_overflow); }
			template<uint> double4 const& get()const { throw_exception(except_code::array_overflow); }
			template<uint> void set(double4 const& value) { throw_exception(except_code::array_overflow); }

			template<uint X, uint Y> double get()const { return get<X>().template get<Y>(); }
			template<uint X, uint Y> void set(double value) { get<X>().template set<Y>(value); }
...
ANG_REGIST_RUNTIME_VALUE_TYPE_INFO_INLINE(ang::maths::float4x4);
ANG_REGIST_RUNTIME_VALUE_TYPE_INFO_INLINE(ang::maths::double4x4);
