...
typedef ang_atomic(dword) ang_atomic_dword_t;

#ifdef WINDOWS_PLATFORM
ang_inline dword ang_atomic_decrement(volatile ang_atomic_dword_t* mem) {
	return InterlockedDecrement(mem);
}

ang_inline dword ang_atomic_increment(volatile ang_atomic_dword_t* mem) {
	return InterlockedIncrement(mem);
}
#elif defined ANDROID_PLATFORM
ang_inline dword ang_atomic_decrement(volatile ang_atomic_dword_t* mem) {
	return atomic_fetch_sub(mem , 1);
}

ang_inline dword ang_atomic_increment(volatile ang_atomic_dword_t* mem) {
	return atomic_fetch_add(mem, 1);
}
#endif
