...
void coffe::collections::vector_buffer<MY_TYPE, MY_ALLOC>::size(wsize size, bool save)
{
	...
	if (size > m_size)
	{
		for (wsize i = m_size; i < size; ++i)
			m_alloc.template construct<MY_TYPE, MY_TYPE const&>((MY_TYPE*)&m_data[i], default_value<MY_TYPE>::value);
	}
	else
	{
		for (wsize i = m_size; i > size; --i)
			m_alloc.destroy((MY_TYPE*)&m_data[i - 1]);
	}
	m_size = size;
}
...
void coffe::collections::vector_buffer<MY_TYPE, MY_ALLOC>::push(MY_TYPE const& value, bool last)
{
	...
	if (last)
	{	
		m_alloc.template construct<MY_TYPE, MY_TYPE const&>((MY_TYPE*)&m_data[m_size++], value);
	}
	else
	{
		for (auto i = m_size; i > 0U; --i)
		{
			m_alloc.template construct<MY_TYPE,MY_TYPE&&>((MY_TYPE*)&m_data[i], coffe::move(m_data[i - 1U]));
			m_alloc.destroy((MY_TYPE*)&m_data[i - 1U]);
		}
		m_alloc.template construct<MY_TYPE,MY_TYPE const&>((MY_TYPE*)&m_data[0], value);
		m_size++;
	}
}
...
bool coffe::collections::vector_buffer<MY_TYPE, MY_ALLOC>::insert(windex idx, MY_TYPE const& value)
{
	...
	m_alloc.template construct<MY_TYPE,MY_TYPE const&>((MY_TYPE*)&m_data[idx], value);
	m_size++;
	return true;
}
...
bool coffe::collections::vector_buffer<MY_TYPE, MY_ALLOC>::pop(bool last)
{
	...
	m_alloc.destroy((MY_TYPE*)&(m_data[--m_size]));
	return true;
}
...
void coffe::collections::vector_buffer<MY_TYPE, MY_ALLOC>::clear()
{
	if (m_data != null)
	{
		for (wsize i = 0; i < m_size; ++i)
			m_alloc.destroy((MY_TYPE*)&m_data[i]);
		m_size = 0U;
		m_capacity = 0U;
...
bool coffe::collections::vector_buffer<MY_TYPE, MY_ALLOC>::realloc(wsize new_size, bool save)
{
	...
	if (save)
	{
		...
		{
			m_alloc.template construct<MY_TYPE,MY_TYPE&&>((MY_TYPE*)&new_buffer[i], coffe::move(m_data[i]));
			m_alloc.destroy((MY_TYPE*)&m_data[i]);
		}
		m_alloc.deallocate(m_data);
	}
