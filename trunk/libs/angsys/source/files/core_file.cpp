...
bool mapped_file_buffer::map(open_flags_t access, wsize size, ulong64 offset)
{
	...
	m_access_flag += access.get() & open_flags_t((open_flags)0X1F);
...
wsize mapped_file_buffer::mem_copy(wsize size, pointer data, text::encoding_t encoding)
{
	...
	if (bool(m_access_flag & open_flags::format_text))
	{
		...
		return text::text_encoder<text::encoding::auto_detect>::convert(dest, src, true).count();
	}
	else
	{
...
ibuffer_view_t mapped_file_buffer::map_buffer(windex start, wsize size)
{
	if ((start + size) >= m_buffer_size)
		return null;
	return null;// new buffer_view(this, start, size);
}
	...
	if (bool(flags.is_active(open_flags::format_text))) //text file
	{
		if (size > 0) 
		{
			...
			case text::encoding::utf32_be:
				...
				break;
			default:
				if ((flags & (open_flags)0X1F) != open_flags::format_text)
					m_flags += flags & (open_flags)0X1F;
				else
...
{
	return new text::basic_string_buffer<path_encoding>(m_path);
}
