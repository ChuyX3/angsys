...
rtti_t const& type_info_builder<void, gender::value_type>::type_of() { 
	static rtti_t const& info = rtti::regist("void"_sv, gender::value_type, sizeof(wsize), alignof(wsize)); 
	return info;
}
...
rtti_t const& rtti::regist(type_name_t name, gender_t g, wsize sz, wsize a)
{
...
rtti_t const& rtti::regist(fast_astring_t&& name, gender_t g, wsize sz, wsize a)
{
	return regist(runtime_type_manager::instance()->push_str(ang::move(name)), g, sz, a);
}

rtti_t const& rtti::regist(fast_astring_t&& name, gender_t g, wsize sz, wsize a, inherit_pack_info_t parents, dynamic_cast_proc cast)
{
	return regist(runtime_type_manager::instance()->push_str(ang::move(name)), g, sz, a, parents, cast);
}