...
bool d3d11::d3d_load_shader_input(collections::vector<graphics::reflect::attribute_desc> const& attributes, collections::vector<D3D11_INPUT_ELEMENT_DESC>& input_layout_desc)
{
	...
	{
		desc.SemanticName = (const char*)(castr_t)attribute.semantic().to_string();
...
bool d3d11::d3d_reflect_shader_variable(reflect::varying_desc_t& field, ID3D11ShaderReflectionVariable* varInfo)
{
	...
	field.var_name(castr_t(varDesc.Name, invalid_index));
...
bool d3d11_shaders::load(d3d11_driver_t driver, string vertex, string pixel, string sid, astring* log)
{
	astring error;
...
bool d3d11_shaders::load(d3d11_driver_t driver, shader_info_t const& vertex, shader_info_t const& pixel, string sid, astring* log)
{
	astring error;
...
astring d3d11_shaders::load_vertex_shader(d3d11_driver_t driver, string shader_info)
{
	...
	if (FAILED(hr = D3DCompile((castr_t)code, code.length()
		...
		, driver->vs_model(), D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &compiled_code, &error
	))) {
...
astring d3d11_shaders::load_vertex_shader(d3d11_driver_t driver, shader_info_t const& shader_info)
{
	astring entry = null;
	vector<astring> configs;
...
astring d3d11_shaders::load_pixel_shader(d3d11_driver_t driver, string shader_info)
{
	astring entry = "main"_sv;
	string filename = null;
	astring config = null;
	...
	if (filename.is_empty())
		return "d3d11_shaders::load_pixel_shader: invalid parameters"_sv;

	core::files::input_text_file_t file;
	driver->get_file_system()->open(filename, &file);
	if (!file->is_valid())
		return "d3d11_shaders::load_pixel_shader: invalid file name"_sv;
	...
	if (FAILED(hr = D3DCompile(code.cstr(), code.length()
		, NULL, macros.data(), NULL, (castr_t)entry
		, driver->ps_model(), D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &compiled_code, &error
	))) {
		return castr_t((const char*)error->GetBufferPointer(), error->GetBufferSize());
	}
	...
	if (FAILED(hr = D3DReflect(
		...
		_com_error error(hr, null);
		return castr_t(error.ErrorMessage(), invalid_index);
	}
...
astring d3d11_shaders::load_pixel_shader(d3d11_driver_t driver, shader_info_t const& shader_info)
{
	astring entry = null;
	vector<astring> configs;
	ID3DBlob* compiled_code = null;
	...
	entry = !shader_info.entry.is_empty() ? shader_info.entry : astring("main"_sv);
...
{
	return m_input_layout.is_empty() ? array_view<reflect::attribute_desc>() : collections::to_array(m_input_layout.begin(), m_input_layout.end());
}

array_view<reflect::varying_desc> d3d11_shaders::vs_uniforms_layouts()const
{
	return m_vs_uniforms.is_empty() ? array_view<reflect::varying_desc>() : collections::to_array(m_vs_uniforms.begin(), m_vs_uniforms.end());
}

array_view<reflect::varying_desc> d3d11_shaders::ps_uniforms_layouts()const
{
	return m_ps_uniforms.is_empty() ? array_view<reflect::varying_desc>() : collections::to_array(m_ps_uniforms.begin(), m_ps_uniforms.end());
}
