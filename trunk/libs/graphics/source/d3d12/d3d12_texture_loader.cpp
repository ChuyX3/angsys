...
#define MAKEFOURCC(ch0, ch1, ch2, ch3)                              \
                ((coffe_uint32_t)(coffe_uint8_t)(ch0) | ((coffe_uint32_t)(coffe_uint8_t)(ch1) << 8) |       \
                ((coffe_uint32_t)(coffe_uint8_t)(ch2) << 16) | ((coffe_uint32_t)(coffe_uint8_t)(ch3) << 24 ))
#endif /* defined(MAKEFOURCC) */


#pragma pack(push,1) // Byte alignment (8-bit)

const coffe_uint32_t DDS_MAGIC = 0x20534444; // "DDS "

struct DDS_PIXELFORMAT
{
	coffe_uint32_t    size;
	coffe_uint32_t    flags;
	coffe_uint32_t    fourCC;
	coffe_uint32_t    RGBBitCount;
	coffe_uint32_t    RBitMask;
	coffe_uint32_t    GBitMask;
	coffe_uint32_t    BBitMask;
	coffe_uint32_t    ABitMask;
};
...
struct DDS_HEADER
{
	coffe_uint32_t        size;
	coffe_uint32_t        flags;
	coffe_uint32_t        height;
	coffe_uint32_t        width;
	coffe_uint32_t        pitchOrLinearSize;
	coffe_uint32_t        depth; // only if DDS_HEADER_FLAGS_VOLUME is set in flags
	coffe_uint32_t        mipMapCount;
	coffe_uint32_t        reserved1[11];
	DDS_PIXELFORMAT		ddspf;
	coffe_uint32_t        caps;
	coffe_uint32_t        caps2;
	coffe_uint32_t        caps3;
	coffe_uint32_t        caps4;
	coffe_uint32_t        reserved2;
};

struct DDS_HEADER_DXT10
{
	DXGI_FORMAT			dxgiFormat;
	coffe_uint32_t        resourceDimension;
	coffe_uint32_t        miscFlag; // see D3D11_RESOURCE_MISC_FLAG
	coffe_uint32_t        arraySize;
	coffe_uint32_t        miscFlags2;
};


struct TGA_HEADER
{
	coffe_uint8_t  identsize;			// size of ID field that follows 18 byte header (0 usually)
	coffe_uint8_t  colourmaptype;		// type of colour map 0=none, 1=has palette
	coffe_uint8_t  imagetype;			// type of image 2=rgb uncompressed, 10 - rgb rle compressed

	coffe_uint16_t colourmapstart;				// first colour map entry in palette
	coffe_uint16_t colourmaplength;				// number of colours in palette
	coffe_uint8_t  colourmapbits;		// number of bits per palette entry 15,16,24,32

	coffe_uint16_t xstart;						// image x origin
	coffe_uint16_t ystart;						// image y origin
	coffe_uint16_t width;						// image width in pixels
	coffe_uint16_t height;						// image height in pixels
	coffe_uint8_t  bits;				// image bits per pixel 24,32
	coffe_uint8_t  descriptor;			// image descriptor bits (vh flip bits)
...
COFFE_REGIST_RUNTIME_VALUE_TYPE_INFO_INLINE(D3D11_SUBRESOURCE_DATA);
	...
	_Out_opt_ wsize* outNumRows)
{
	...
	if (bc)
	{
		...
		if (width > 0)
		{
			numBlocksWide = coffe::max<wsize, wsize>(1, (width + 3) / 4);
		}
		wsize numBlocksHigh = 0;
		if (height > 0)
		{
			numBlocksHigh = coffe::max<wsize, wsize>(1, (height + 3) / 4);
		}
...
bool FillInitDataFromDDS(coffe::ibuffer_t in_data, tex_file_info_t const* header, wsize& twidth, wsize& theight, wsize& tdepth, wsize& skipMip, coffe::array<D3D11_SUBRESOURCE_DATA>& out_data)
{
	using namespace coffe;
	...
	out_data = new collections::array_buffer<D3D11_SUBRESOURCE_DATA>(header->arraySize * header->mipCount);
...
bool FillInitDataFromTGA(coffe::ibuffer_t in_data, d3d11::tex_file_info_t const* header, coffe::array<D3D11_SUBRESOURCE_DATA>& init_data, coffe::array<coffe::ibuffer_t>& out_data)
{
	if (header->isCubeMap)
	{
		auto w = header->width / 4;

		out_data = NEW coffe::collections::array_buffer<coffe::ibuffer_t>{
			new(4 * w * w) coffe::buffer(),
			new(4 * w * w)coffe::buffer(),
			new(4 * w * w)coffe::buffer(),
			new(4 * w * w)coffe::buffer(),
			new(4 * w * w)coffe::buffer(),
		...
		init_data = NEW coffe::collections::array_buffer<D3D11_SUBRESOURCE_DATA>(6);

		memset(init_data->buffer_ptr(), 0, init_data->buffer_size());
	...
	{
		init_data = NEW coffe::collections::array_buffer<D3D11_SUBRESOURCE_DATA>(1);
...
coffe::ibuffer_t d3d11_texture_loader::load_dds(coffe::core::files::input_binary_file_t file, d3d11::tex_file_info_t& dds_header)
{
	using namespace coffe;

	coffe::ibuffer_t outData = null;
	wsize dataSize = (wsize)file->size();
	int offset = sizeof(DDS_HEADER) + sizeof(coffe_uint32_t);
	...
	file->read([&](streams::ibinary_input_stream_t stream)->bool
	{
		...
		outData = new(dataSize - offset) coffe::buffer();
...
coffe::ibuffer_t d3d11_texture_loader::load_tga(coffe::core::files::input_binary_file_t file, uint& width, uint& height, uint& format)
{
	coffe::ibuffer_t out_data = null;
	file->read([&](coffe::streams::ibinary_input_stream_t stream)->bool
	{
		TGA_HEADER header;
		stream >> header;
		...
		if (header.bits != 24 && header.bits != 32)
			return false;
		stream->cursor(header.identsize, coffe::streams::stream_reference::current);
		...
		out_data = new(header.width * header.height * 4) coffe::buffer();
...
bool d3d11_texture_loader::create_texture(d3d11_driver_t driver, d3d11::tex_file_info_t const* info, coffe::ibuffer_t buffer, ID3D11Resource** resource, ID3D11ShaderResourceView** resourceView)
{
	...
	coffe::array<coffe::ibuffer_t> out_data;
	coffe::array<D3D11_SUBRESOURCE_DATA> init_data;
...
bool d3d11_texture_loader::create_array_texture(d3d11_driver_t driver, tex_file_info_t const* info, coffe::collections::vector<coffe::ibuffer_t>const& buffers, ID3D11Resource** resource, ID3D11ShaderResourceView** resourceView)
{
	if (info->resDim != D3D11_RESOURCE_DIMENSION_TEXTURE2D)
		return false;
	...
	{
		init_data = new collections::array_buffer<D3D11_SUBRESOURCE_DATA>(info->arraySize);
...
bool d3d11_texture_loader::load_texture(d3d11_driver_t driver, coffe::core::files::input_binary_file_t file, tex_file_info_t& info, ID3D11Resource** resource, ID3D11ShaderResourceView** resourceView, bool isCube)
{
...
bool d3d11_texture_loader::load_texture(d3d11_driver_t driver,
	coffe::collections::vector<coffe::core::files::input_binary_file_t> const& files, tex_file_info_t& info,
	ID3D11Resource** resource, ID3D11ShaderResourceView** resourceView, bool isCube)
{
	if (resource == null && resourceView == null)
		return false;

	coffe::collections::vector<ibuffer_t> data;

	ZeroMemory(&info, sizeof(info));

	for(coffe::core::files::input_binary_file_t file : files)
	{
		file->read(&info.fileType, sizeof(info.fileType));
		if (info.fileType == ' SDD')
		{
	...
	if (data.is_empty() || (isCube && data->counter() != 6))
		return false;
