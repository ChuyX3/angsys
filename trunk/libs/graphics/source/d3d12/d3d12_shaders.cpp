...
using namespace coffe::graphics::effects;
...
bool d3d11::d3d_reflect_shader_variable(reflect::varying_desc_t& field, ID3D11ShaderReflectionVariable* varInfo)
{
	...
	case D3D_SVC_STRUCT: {
		...
		for (windex i = 0; i < typeDesc.Members; i++) {
			...
			field.fields(coffe::move(fields), false);
		}
		return true;
	}
...
bool d3d11::d3d_reflect_shader_uniforms(reflect::uniform_fields_t& uniforms, ID3D11ShaderReflection* vertexShaderReflection)
{
	...
	for (windex i = 0; i < reflectDesc.ConstantBuffers; ++i)
	{
		...
		for (windex j = 0; j < desc.Variables; j++)
		{
			...
			fields += coffe::move(field);
		}
		cbuffer.fields(coffe::move(fields), false);
		cbuffer.aligment(16);
		uniforms += coffe::move(cbuffer);
	}
	return true;
}
...
COFFE_IMPLEMENT_OBJECT_RUNTIME_INFO(coffe::graphics::d3d11::d3d11_shaders);
COFFE_IMPLEMENT_OBJECT_CLASS_INFO(coffe::graphics::d3d11::d3d11_shaders, system_object, ishaders, resources::iresource);
COFFE_IMPLEMENT_OBJECT_QUERY_INTERFACE(coffe::graphics::d3d11::d3d11_shaders, system_object, ishaders, resources::iresource);
...
string d3d11_shaders::load_vertex_shader(d3d11_driver_t driver, string shader_info)
{
	...
	macros += {(const char*)(castr_t)driver_type, null};
	if (!config.is_empty())
	{
		configs = coffe::move(config->split("-D"));
...
string d3d11_shaders::load_vertex_shader(d3d11_driver_t driver, shader_info_t const& shader_info)
{
	...
	if (shader_info.code.is_empty())
	{
		...
		if (!file->map([&](ibuffer_view_t code)
		{
			...
			castr_t log = error?(const char*)error->GetBufferPointer(): "d3d11_shaders::load_vertex_shader: unknown error";
			return log;
		}
	}
	else
...
bool d3d11_shaders::load_vs_const_buffer(d3d11_driver_t driver, graphics::reflect::varying_desc& uniforms)
{
	...
	m_vs_uniforms += coffe::move(uniforms);
	return false;
}
...
bool d3d11_shaders::load_ps_const_buffer(d3d11_driver_t driver, graphics::reflect::varying_desc& uniforms)
{
	...
	m_ps_uniforms += coffe::move(uniforms);
	return false;
}
...
reflect::varying d3d11_shaders::map_vs_uniform(idriver_t, windex idx)
{
	...
	desc.position(idx);
	return reflect::varying(buff, coffe::move(desc));
}
