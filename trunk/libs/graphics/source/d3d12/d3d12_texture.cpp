...
COFFE_IMPLEMENT_OBJECT_RUNTIME_INFO(coffe::graphics::d3d11::d3d11_texture);
COFFE_IMPLEMENT_OBJECT_CLASS_INFO(coffe::graphics::d3d11::d3d11_texture, system_object, textures::itexture, resources::iresource);
COFFE_IMPLEMENT_OBJECT_QUERY_INTERFACE(coffe::graphics::d3d11::d3d11_texture, system_object, textures::itexture, buffers::igpu_buffer, resources::iresource);
...
intfptr d3d11_texture::fast_cast(resources::resource_type_t type) {
...
bool d3d11_texture::create(d3d11_driver_t driver, textures::tex_type_t type, textures::tex_format_t color_format, buffers::buffer_usage_t usage, buffers::buffer_bind_flag_t flags, size3d<uint> dimentions, string sid)
{
	...
	case textures::tex_type::tex1D: {
		...
		desc.Width = max(1, dimentions.width);
		desc.Format = format;
		desc.MipLevels = 1;
		desc.ArraySize = max(1,  dimentions.depth);
		...
		srvdesc.ViewDimension = (dimentions.depth > 1 ? D3D11_SRV_DIMENSION_TEXTURE1DARRAY : D3D11_SRV_DIMENSION_TEXTURE1D);
	...
	case textures::tex_type::tex2D: {
		...
		ZeroMemory(&desc, sizeof(desc));
		desc.Height = max(1, dimentions.height);
		desc.Width = max(1, dimentions.width);
		desc.Format = format;
		desc.MipLevels = 1;
		desc.ArraySize = max(1,dimentions.depth);
		...
		srvdesc.ViewDimension = (dimentions.depth > 1? D3D11_SRV_DIMENSION_TEXTURE2DARRAY : D3D11_SRV_DIMENSION_TEXTURE2D);
	...
	case textures::tex_type::tex3D: {
		...
		desc.Height = max(1, dimentions.height);
		desc.Width = max(1, dimentions.width);
		desc.Depth = max(1, dimentions.depth);
		desc.Format = format;
		desc.MipLevels = 1;
		desc.Usage = (D3D11_USAGE)usage.get(); //D3D11_USAGE_DEFAULT
		desc.BindFlags = flags; //D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_RENDER_TARGET;
...
bool d3d11_texture::attach(com_wrapper<ID3D11Resource> resource, d3d11_driver_t driver, string sid)
{
	...
	case D3D11_RESOURCE_DIMENSION_TEXTURE1D: {
		...
		m_tex_dimentions = { tex_desc.Width, 1, tex_desc.ArraySize };
	} break;
	case D3D11_RESOURCE_DIMENSION_TEXTURE2D: {
		...
		m_usage = (buffers::buffer_usage)tex_desc.Usage;
		m_bind_flags = (buffers::buffer_bind_flag)tex_desc.BindFlags;
		m_tex_dimentions = { tex_desc.Width, tex_desc.Height, tex_desc.ArraySize };
