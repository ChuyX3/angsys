...
namespace ang
{
	namespace runtime
	{
		...
		inline astring& args_list_type_name(astring& out);

		template<typename T, typename... Ts>
		inline astring& args_list_type_name_builder(astring& out) {
		...
		struct __type_info_builder_genre_value_helper<const T> {
			static rtti_t const& type_of() {
				...
				rtti_t const& info = rtti::regist(
					text::string_pool::instance()->save_string(astring("const "_s) += rtti::type_of<type>().type_name())
		...
		struct __type_info_builder_genre_value_helper<T&> {
			static rtti_t const& type_of() {
				typedef typename remove_reference<typename remove_constant<T>::type>::type type;
				rtti_t const& info = rtti::regist(
					text::string_pool::instance()->save_string(astring(rtti::type_of<type>().type_name()) += "&"_s)
					, ang::genre::value_type, sizeof(type), alignof(type));
				return info;
			}
		};
		...
		struct __type_info_builder_genre_value_helper<const T&> {
			static rtti_t const& type_of() {
				typedef typename remove_reference<typename remove_constant<T>::type>::type type;
				rtti_t const& info = rtti::regist(
					text::string_pool::instance()->save_string((astring("const "_s) += rtti::type_of<type>().type_name()) += "&"_s)
					, ang::genre::value_type, sizeof(type), alignof(type));
				return info;
			}
		};
		...
		struct __type_info_builder_genre_value_helper<T*> {
			static rtti_t const& type_of() {
				typedef typename remove_reference<typename remove_constant<T>::type>::type type;
				rtti_t const& info = rtti::regist(
					text::string_pool::instance()->save_string(astring(rtti::type_of<T>().type_name()) += "*"_s)
					, ang::genre::value_type, sizeof(type*), alignof(type*));
				return info;
			}
		};
		...
		struct __type_info_builder_genre_function_helper<T(__stdcall*)(Ts...)> {
			static rtti_t const& type_of() {
				rtti_t const& info = rtti::regist(
					text::string_pool::instance()->save_string(((astring(rtti::type_of<T>().type_name()) += "(__stdcall*)("_s) += runtime::args_list_type_name<Ts...>()) += ")"_s)
					, genre::function_type, sizeof(wsize), alignof(wsize));
				return info;
			}
		};
		...
		struct __type_info_builder_genre_function_helper<T(O::*)(Ts...)> {
			static rtti_t const& type_of() {
				rtti_t const& info = rtti::regist(
					text::string_pool::instance()->save_string(((((astring(rtti::type_of<T>().type_name()) +="("_s ) += rtti::type_of<O>().type_name()) += "::*)("_s) += runtime::args_list_type_name<Ts...>()) += ")"_s)
					, genre::function_type, sizeof(wsize), alignof(wsize));
				return info;
			}
		};
