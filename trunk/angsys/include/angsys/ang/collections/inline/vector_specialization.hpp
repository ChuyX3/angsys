...
namespace ang
{
	namespace collections
	{
		...
		class MY_LINKAGE vector_buffer<MY_TYPE, MY_ALLOC > final
			: public smart<vector_buffer<MY_TYPE, MY_ALLOC>, object, ivariant, ibuffer, ilist<MY_TYPE>>
		{
		public:
			using type							= MY_TYPE;
		...
		public:
			...
			template<typename U>inline vector_buffer(ang::initializer_list<U> items)
				: vector_buffer() {
	...
	public:
		...
		inline collections::forward_iterator<const data_type> begin()const { return m_ptr ? m_ptr->begin() : collections::iterator<const data_type>(); }
		inline collections::forward_iterator<const data_type> end()const { return m_ptr ? m_ptr->end() : collections::iterator<const data_type>(); }
