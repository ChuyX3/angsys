...
void ang::collections::vector_buffer<MY_TYPE, MY_ALLOC>::push(MY_TYPE const& value, bool last)
{
	...
	else
	{
		...
		{
			m_alloc.template construct<MY_TYPE,MY_TYPE&&>((MY_TYPE*)&m_data[i], ang::move(m_data[i - 1U]));
...
bool ang::collections::vector_buffer<MY_TYPE, MY_ALLOC>::realloc(wsize new_size, bool save)
{
	...
	if (save)
	{
		...
		{
			m_alloc.template construct<MY_TYPE,MY_TYPE&&>((MY_TYPE*)&new_buffer[i], ang::move(m_data[i]));
			m_alloc.destroy((MY_TYPE*)&m_data[i]);
		}
		m_alloc.deallocate(m_data);
	}
