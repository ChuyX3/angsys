...
bool FillInitDataFromTGA(ang::ibuffer_t in_data, d3d11::tex_file_info_t const* header, ang::array<D3D11_SUBRESOURCE_DATA>& init_data, ang::array<ang::ibuffer_t>& out_data)
{
	if (header->isCubeMap)
	{
		...
		out_data = NEW ang::collections::array_buffer<ang::ibuffer_t>{
			new(4 * w * w) ang::buffer(),
			new(4 * w * w)ang::buffer(),
			new(4 * w * w)ang::buffer(),
			new(4 * w * w)ang::buffer(),
			new(4 * w * w)ang::buffer(),
		...
		init_data = NEW ang::collections::array_buffer<D3D11_SUBRESOURCE_DATA>(6);
	...
	{
		init_data = NEW ang::collections::array_buffer<D3D11_SUBRESOURCE_DATA>(1);
...
ang::ibuffer_t d3d11_texture_loader::load_tga(ang::core::files::input_binary_file_t file, uint& width, uint& height, uint& format)
{
	...
	file->read([&](ang::streams::ibinary_input_stream_t stream)->bool
	{
		...
		out_data = new(header.width * header.height * 4) ang::buffer();
