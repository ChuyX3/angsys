...
template<> ang::maths::float2 ang::dom::xml::xml_text::xml_as<ang::maths::float2>()const {
	cstr_t cstr = xml_as<cstr_t>();
	if (cstr.ptr() == null)
		return{ 0,0 };
...
template<> ang::maths::float3 ang::dom::xml::xml_text::xml_as<ang::maths::float3>()const {
	cstr_t cstr = xml_as<cstr_t>();
	if (cstr.ptr() == null)
		return{ 0,0 };
	text::iparser_t parser = text::iparser::get_parser(cstr.encoding());
...
template<> ang::maths::float4 ang::dom::xml::xml_text::xml_as<ang::maths::float4>()const {
	cstr_t cstr = xml_as<cstr_t>();
	if (cstr.ptr() == null)
		return{ 0,0 };
	text::iparser_t parser = text::iparser::get_parser(cstr.encoding());
...
bool scene::load(idriver_t driver, effects::ieffect_library_t fxlibrary, textures::itexture_loader_t texloader, dom::xml::xml_node_t scene_info)
{
	...
	for(dom::xml::xml_node_t node : scene_info->xml_children())
	{
		...
		{
			auto type = node->xml_attributes()["type"_s]->xml_as<cstr_t>();
			if (type == "camera"_s)
			{
				if (m_camera == null)m_camera = new scenes::camera();
				if (!m_camera->load(this, node))
...
bool scene::load_sources(dom::xml::xml_node_t sources_info)
{
	...
	for(dom::xml::xml_node_t file : sources_info->xml_children())
	{
		...
		if (name != "file"_s)
			continue;
		m_mutex.lock();
		m_source_map += {file->xml_attributes()["sid"_s]->xml_as<cstr_t>(), file->xml_value()->xml_as<cstr_t>()};
		m_mutex.unlock();
	}
	return true;
}
