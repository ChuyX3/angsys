...
private:
	int m_value;
	static int get_value(base_property<int>const* prop) {
		return forward<int>(field_to_parent(&A::value, prop)->m_value);
	}
	static void set_value(base_property<int> const* prop, int&& val) {
		field_to_parent(&A::value, prop)->m_value = forward<int>(val);
	}

	string m_name;
	static string get_name(base_property<string>const* prop) {
		return field_to_parent(&A::name, prop)->m_name.get();
	}
	static void set_name(base_property<string> const* prop, text::raw_cstr_t val) {
		field_to_parent(&A::name, prop)->m_name = val;
	}

public:
	property<const int, get_value> value;
	property<text::string_buffer, get_name, set_name> name;

	
	A()
		: m_value(0)
		, m_name("Jesus Rocha"_s)
	{
