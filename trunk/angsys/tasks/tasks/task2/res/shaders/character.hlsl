...
float3 get_light_difusse(float3 normal, float3 world_pos)
{
	...
	for (uint i = 0; i < light_count; ++i)
	{
		...
		if (intensity > 0.95)
			...
			difusse = float3(0.2, 0.2, 0.2) * light_data[i].color.xyz;
		else
			difusse = float3(0.01, 0.01, 0.01) * light_data[i].color.xyz;
#else
...
#ifdef LIGHTING
	float3 total = (ambient_color.xyz * ambient_color.w + difusse) * color.xyz;
	if (specular_power > 0)
		total = saturate(total + get_light_specular(normal, input.vpos_world.xyz));
	color.xyz = total;
#endif
