...
bool xml_tree_iterator::begin_child()
{
	if (xml_current())
	{
		xml_child((xml_current()->xml_children()) ? *xml_current()->xml_children()->begin() : null);
...
{
	if (className == type_of<xml_tree>()
		|| object::is_child_of(className)
		|| className == type_of<ixmlobject>()
		|| className == type_of<collections::ienum<xml_node>>()
		|| className == type_of<collections::ienum<xml_node>>())
		return true;
	return false;
}
...
bool xml_tree::query_object(type_name_t className, unknown_ptr_t out)
{
	...
	else if (ienum::query_object(className, out))
	{
		return true;
	}
...
xml_node_t xml_tree::at(xml_base_iterator_t const& it)
{
...
bool xml_tree::increase(xml_base_iterator_t& it)const
{
	if (!counter() || it.parent() != this || it.current() == null) return false;
	xml_node_t node = reinterpret_cast<xml_node*>(it.current());
	auto children = node->xml_children();
	if (children && children->counter())
	{
		it = xml_iterator_t(const_cast<xml_tree*>(this), *children->begin());
		return true;
	}
...
bool xml_tree::increase(xml_base_iterator_t& it, int offset)const
{
	int c = 0;
	while (c < offset)
	{
		if (!increase(it))return false;
		c--;
	}
	return true;
}

bool xml_tree::decrease(xml_base_iterator_t& it)const
{
	if (!counter() || it.parent() != this || it.current() == null) return false;
	xml_node_t node = reinterpret_cast<xml_node*>(it.current());
...
bool xml_tree::decrease(xml_base_iterator_t& it, int offset)const
{
	int c = 0;
	while (c < offset)
	{
		if (!decrease(it))return false;
		c--;
	}
	return true;
}

xml_forward_iterator_t xml_tree::begin()
{
...
xml_backward_iterator_t xml_tree::rbegin()
{
...
xml_iterator_t xml_tree::find(xml_node_t node)const
{
	for (xml_node_t item : *this)
	{
		if (node.get() == item.get())
			return xml_iterator_t(const_cast<xml_tree*>(this), item);
	}
	return end();
}
...
xml_iterator_t xml_tree::find_next(cstr_t name, xml_iterator_t nextTo, bool invert)const
{
	...
	if (invert)
	{
		for (xml_backward_iterator_t it = nextTo; it.is_valid(); it--)
		{
			if (name == (wstring const&)it->xml_name())
				return it;
		}
	...
	{
		for (xml_forward_iterator_t it = nextTo; it.is_valid(); it++)
		{
			if (name == (wstring const&)it->xml_name())
				return it;
		}
