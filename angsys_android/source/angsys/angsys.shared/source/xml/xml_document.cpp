...
{
	if (code.size() < 4)
		throw exception(xml_exception_code::parsing_error, "invalid input");
	mbyte bum;
	bum.bytes[0] = code[0];
	bum.bytes[1] = code[1];
	bum.bytes[2] = code[2];
	...
	if (!decode_element(code, begin))
	{
		...
		throw exception(xml_exception_code::parsing_error, ang::move(error));
	}
}
...
{
	if (code.size() < 4)
		throw exception(xml_exception_code::parsing_error, "invalid input"_s);
